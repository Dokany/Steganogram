        ...
        case ViewsRequest:
        {
            ...
            if(accept)
            {
               ...
               // currentImageViewers.erase(name);
                ...
                bool found=false;
                for(int i=0;i<currentImageViewers[name].size() && !found;i++)
                {
                    if(currentImageViewers[name][i].first==currentOnlineUsers[ip].first)
                    {
                        currentImageViewers[name][i].second = mbox_count;
                        found=true;
                    }
                }
                if(!found)currentImageViewers[name].push_back(make_pair(currentOnlineUsers[ip].first,mbox_count));
...
void Peer::handleReceivedMessage(Message m, string id)
{
    ...
    switch(mt)
    {
        ...
        case ImageReply:
        {
            ...
            out.close();
            ImageData id2;
            id2.setPath(shared_directory);
            id2.setName(name);
            id2.embeddInDefault();

            int count = id2.getCount();
            mbox_count = count;
...
void Peer::revokeImage(string name, string user)
{
    ImageData id(name,"Images/",0);
    Message tst(ImageReply, string(myHostname), myPort, nameToAddress[user].first, nameToAddress[user].second);
    tst.setData(id);
    tst.Flatten();
    execute(tst);
    string m_id = tst.getID()+tst.getOwnerIP();
...
string Peer::viewImage(string path)
{
    ...
    string tt = tmp.view(path,mbox_count);
    cout<<"got count in viewImage "<<mbox_count<<endl;
    return tt;
}
